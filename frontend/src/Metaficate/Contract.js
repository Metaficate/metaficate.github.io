 ...
 */
class ContractManager extends MetaMaskManager {
  ...
  checkContractRead() {
    if (!this.contract) throw Error('Not connected')
  }

  checkContractWrite() {
    if (!this.contract || !this.account) throw Error('Not connected')
  }
}
...
export class MeteficateContract extends ContractManager {
  ...
  async tokenInfo(tokenId) {
    ...
    if (typeof tokenId !== 'number') {
      console.error('tokenId type is wrong', tokenId)
      throw Error('Query token failed, invalid tokenId')
    }
