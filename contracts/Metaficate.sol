...
contract Metaficate is ERC721 {
    ...
    function createIcon(uint8 color_index) public view returns (string memory) {
    ...
    function createImage(uint256 tokenId) public view returns (string memory) {
        string memory addr = addressToString(tokenInfoMap[tokenId].addr);
        ...
        uint8 color_index = uint8(tokenId & 63); // 0b111111
        uint8 text_index = uint8(tokenInfoMap[tokenId].ts & 7); // 0b111
        ...
        string memory icon = createIcon(color_index);
